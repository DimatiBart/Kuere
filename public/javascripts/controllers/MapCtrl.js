  ...
  $scope.showMarkers = function(){
   leafletData.getMap().then(function(map) {
    for (var i in $scope.posts) {
     ...
     $scope.markers[i] = {
      lat: $scope.posts[i].coordinates.lat,
      lng: $scope.posts[i].coordinates.lng,
      message: type + '  ' + '<a href="#/post/'+$scope.posts[i].id+'">' + $scope.posts[i].title + '</a>',
      riseOnHover: true
